(ns kanjo.methods.social
  "social.cljc — 勘定 (かんじょう) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kanjo's own
...
(defn draft-observation-post
  "A dry-run observation post over kanjo's own domain (Datafies the world's PUBLIC (listed) companies' DISCLOSED financial facts — the balance sheet (貸借対照表), income statement (損益計算書) and…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kanjo R0: live social posting is Council Lv6+ + operator + "
